...
{
public:
  GradTrajOptimizer(Configs cfg, const vector<Eigen::Vector3d> &waypoints);
  ...
  void getPVAFromCoeff(vector<Eigen::Vector3d> &pva, const Eigen::MatrixXd &coeff, const int &index,
                            const double &time);
private:
...
GradTrajOptimizer::GradTrajOptimizer(Configs cfg,
                                     const vector<Eigen::Vector3d> &way_points)
{
  ...
  this->retry_offset = cfg.retry_offset;
  this->time_limit_1 = cfg.time_limit_1;
  this->time_limit_2 = cfg.time_limit_2;
  this->try_limit = cfg.try_limit;
  this->deltat = cfg.deltat;

  this->w_smooth = cfg.w_smooth;
  this->w_collision = cfg.w_collision;
  this->w_collision_temp = cfg.w_collision_temp;
  this->sgm_time = cfg.sgm_time;
  this->mean_v = cfg.mean_v;
  this->init_time = cfg.init_time;

  this->alpha = cfg.alpha;
  this->d0 = cfg.d0;
  this->r = cfg.r;

  this->alphav = cfg.alphav;
  this->v0 = cfg.v0;
  this->rv = cfg.rv;

  this->alphaa = cfg.alphaa;
  this->a0 = cfg.a0;
  this->ra = cfg.ra;
  ...
  this->gd_value = cfg.gd_value;
  this->gd_type = cfg.gd_type;
...
void GradTrajOptimizer::getPVAFromCoeff(vector<Eigen::Vector3d> &pva, const Eigen::MatrixXd &coeff, const int &index,
                            const double &time) {                        
  getPositionFromCoeff(pva[0], coeff, index, time);

  getVelocityFromCoeff(pva[1], coeff, index, time);

  getAccelerationFromCoeff(pva[2], coeff, index, time);
}
...
void GradTrajOptimizer::tryDifferentParameter()
{
  ...
  srand(int(tn.tv_sec*1e12 + tn.tv_nsec) % int(2e7));//ros::Time::now().toNSec()
...
bool GradTrajOptimizer::optimizeTrajectory(int step)
{
  ...
  struct timespec tn;
  clock_gettime(CLOCK_REALTIME, &tn);
  int seed = int(tn.tv_sec*1e12 + tn.tv_nsec) % 65536;
  nlopt::srand(seed);
      ...
      for(int s = 0; s < Time.size(); s++)
      {
        ...
        for(double t = 1e-3; t < Time(s); t += dt)
        {
          ...
          // ------------------------ gradient of collision cost------------------------
          {
            for(int k = 0; k < 3; k++)
            {
              g_colli.row(k) = g_colli.row(k)*0;/* + (gd * grad(k) * cd * vel_norm * T * Ldp +
